...
//        mgr.set(AlarmManager.RTC, System.currentTimeMillis()+1000, restartIntent);
        ...
        Intent intent = new Intent(mContext,Main2Activity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
...
//        android.os.Process.killProcess(android.os.Process.myPid());//再此之前可以做些退出等操作
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(mContext, "很抱歉,程序出现异常,日志将自动上传,即将退出.", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
        }.start();
        //保存日志文件
